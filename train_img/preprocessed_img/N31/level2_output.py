...
def calFeaturePoint(img,lt_x,lt_y,rd_x,rd_y,rate):
    ...
    Lm = cropfaceOrgan(img,level2_pts3,rate,side,lt_x,lt_y)
    return level2_pts3,side,Lm * 255
...
if __name__ == "__main__":
    ...
    for line in lines:
        ...
        point_x = ((float(words[9])-lt_x) * (39.0/side) - (point[0] - 39 * rate)) * (15 / (39 * rate * 2))
        point_y = ((float(words[10])-lt_y) * (39.0/side) - (point[1] - 39 * rate)) * (15 / (39 * rate * 2))
    ...
    for line in lines:
        ...
        point_x = ((float(words[9])-lt_x) * (39.0/side) - (point[0] - 39 * rate)) * (15 / (39 * rate * 2))
        point_y = ((float(words[10])-lt_y) * (39.0/side) - (point[1] - 39 * rate)) * (15 / (39 * rate * 2))
	if point_x > 0 and point_x < 15 and point_y > 0 and point_y < 15:
            fid.write(str(count)+'.jpg')
            fid.write('\t' + str(point_x))
            fid.write('\t' + str(point_y) + '\n')
            img = cv2.imread(filename)
